...
> [!NOTE]
> 이 항목의 정보는 Intune 소프트웨어 클라이언트를 사용하여 PC를 관리하는 Windows 데스크톱에만 적용됩니다. 모바일 디바이스로 등록된 Windows PC의 방화벽 설정을 관리하려면 [Intune에서 엔드포인트 보호 설정 추가](../protect/endpoint-protection-configure.md)를 참조하세요.
